# IV2-6B ActivityNet finetune (BLiM-style).
# Mirrors didemo_blim/config.py — only data section + max_txt_l differ.
# Launch:
#   bash scripts/finetuning/stage2/6B/activitynet_blim/run.sh
#   SMOKE=1 bash <kit>/train_iv2_6b.sh activitynet
import os as __os
...
_ANNO_DIR = __os.environ.get(
    "BLIM_ANET_ANNO_DIR",
    "/data5/ucjung/PoLaRT/Internvideo2_finetune/data/ActivityNet",
)
...
train_file = dict(
    anno_path=f"{_ANNO_DIR}/anet_ret_train.json",
...
test_file = dict(anet_ret_val=dict(
    anno_path=f"{_ANNO_DIR}/anet_ret_val_1.json",
    data_root=_VIDEO_DIR,
    media_type="video",
    is_paragraph_retrieval=True,
    max_txt_l=150,
